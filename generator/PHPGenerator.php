...
class PHPGenerator
{
    ...
    /**
     * Fixed priority of function parameters, undefined will be added after defined
     *
     * @var array
     */
    ...
    private function _generatePhpFunction($name, $data)
    {
        ...
        foreach ($this->_sortFunctionParameters($data['REQUEST PARAMETERS']) as $p) {
    ...
    /**
     * Sort function parameters by predefined order
     *
     * @param array $params
     */
    private function _sortFunctionParameters($params)
    {
        $sorted = [];
        foreach ($this->_paramPriority as $preferredField) {
            foreach ($params as $i => $param) {
                if ($param['name'] == $preferredField) {
                    $sorted[] = $param;
                    unset($params[$i]);
                }
            }
        }
        foreach ($params as $noPref) {
            $sorted[] = $noPref;
        }
